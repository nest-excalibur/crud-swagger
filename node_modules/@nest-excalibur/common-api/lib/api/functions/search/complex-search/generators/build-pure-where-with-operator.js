...
exports.compressKey = exports.buildPureWhereWithOperator = void 0;
const in_reductor_1 = require("../reductors/in-reductor");
function buildPureWhereWithOperator(attribute, valueWithOperator, entityName, index = 1) {
    const operator = Object.keys(valueWithOperator)[0];
    ...
    let valuesToCompare = value;
    if (!valueIsArray) {
        valuesToCompare = [value];
    }
    const pureWhere = { where: '', parameters: {} };
    switch (operator) {
        case '$in':
            pureWhere.where = `${entityName}.${attribute} In(${in_reductor_1.inReductor(valuesToCompare)})`;
            return pureWhere;
        case '$nin':
            pureWhere.where = `${entityName}.${attribute} Not In(${in_reductor_1.inReductor(valuesToCompare)})`;
            return pureWhere;
        case '$btw':
...
exports.buildPureWhereWithOperator = buildPureWhereWithOperator;
function compressKey(index, entityName, attribute) {
